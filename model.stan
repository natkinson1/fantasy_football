data {
    ...
    int y[nplayer, nmatch, 4];
    vector[4] alpha;
}
parameters {
    simplex[4] theta[nplayer];
}
transformed parameters {
    simplex[4] theta_mins[nplayer, nmatch];
    for (i in 1:nplayer) {
        for (j in 1:nmatch) {
            theta_mins[i, j][1] = theta[i][1] * (minutes[i, j] / 90);
            theta_mins[i, j][2] = theta[i][2] * (minutes[i, j] / 90);
            theta_mins[i, j][3] = theta[i][3] * (minutes[i, j] / 90);
            theta_mins[i, j][4] = theta[i][4] * (minutes[i, j] / 90) + (90 - minutes[i, j]) / 90;
        }
    }
}
model {
